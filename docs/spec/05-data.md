...
## topics-industry-matrix.json — v2 (2026-06-14)
...
{
  "companies": [
    { "name": "スターバックス", "subcategory": "グローバルチェーン" },
    { "name": "ブルーボトルコーヒー", "subcategory": "スペシャルティロースター・米国" }
  ],
...
| フィールド | 型 | 説明 |
|-----------|-----|------|
| `companies` | `CompanyEntry[]` | 企業・ブランドエントリの一覧 |
| `companies[].name` | `string` | 企業・ブランド名（記事生成に使用） |
| `companies[].subcategory` | `string` | サブカテゴリ（管理・フィルタリング用） |
| `angles` | `string[]` | ビジネス切り口の一覧 |

**サブカテゴリ一覧（2026-06-14時点）**
...
**組み合わせ数**: `companies.length × angles.length`（68社 × 20切り口 = 1,360件）

**マスタ追加ルール**
- 企業・ブランドは一意の表記で統一する（例: `Hario（ハリオ）` で固定）
- `subcategory` は上記サブカテゴリ一覧の値を使用する（新設する場合は一覧を更新すること）
...
factRisk: "low" | "medium" | "high"              # fact-check.ts が付与。未チェック時は省略
factFlags: ["unverifiable-claim"]                 # 検出フラグ一覧。フラグなしは []
factCheckedAt: "2026-06-14T10:00:00Z"             # チェック実行日時（ISO 8601）
formatIssues: ["too-short", "no-heading"]         # format-check.ts が付与。問題なしは []
formatCheckedAt: "2026-06-14T10:00:00Z"           # チェック実行日時（ISO 8601）
sources:                                          # source-citation.ts が付与
  - "https://example.com/news"
  - "AI生成（GitHub Models / GPT-4o）"
sourceAddedAt: "2026-06-14T10:00:00Z"             # ソース付与日時（ISO 8601）
---
...
| 2026-06-14 | v7 | `topics-industry-matrix.json` を大幅拡充（68社×20切り口=1,360件）。`companies` を `CompanyEntry[]`（`name`+`subcategory`）に変更し11サブカテゴリで整理。`types/index.ts` に `CompanyEntry` 型を追加。 |
